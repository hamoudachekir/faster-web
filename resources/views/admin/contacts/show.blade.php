...
<div class="max-w-4xl">
    ...
    <div class="bg-white rounded-2xl shadow-lg p-8 mb-6">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-3xl font-bold text-gray-900">{{ $application->first_name }} {{ $application->last_name }}</h2>
            <span class="px-4 py-2 text-sm font-bold rounded-full
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <h3 class="font-semibold text-gray-700 mb-2">🚗 Type de service</h3>
                <p class="text-gray-900">{{ $application->service_type }}</p>
            </div>
            <div>
                <h3 class="font-semibold text-gray-700 mb-2">🚙 Type de véhicule</h3>
                <p class="text-gray-900">{{ $application->vehicle_type }}</p>
            </div>
            <div>
                <h3 class="font-semibold text-gray-700 mb-2">🆔 Numéro de permis</h3>
                <p class="text-gray-900">{{ $application->license_number }}</p>
            </div>
            <div>
                <h3 class="font-semibold text-gray-700 mb-2">🚗 Possède un véhicule</h3>
                <p class="text-gray-900">{{ $application->has_vehicle ? 'Oui' : 'Non' }}</p>
            </div>
            <div>
        ...
        <div class="mt-6 pt-6 border-t border-gray-200">
            <h3 class="font-semibold text-gray-700 mb-2">💬 Message du candidat</h3>
            <p class="text-gray-900 whitespace-pre-wrap">{{ $application->message }}</p>
        </div>
        @endif
    ...
    <div class="bg-white rounded-2xl shadow-lg p-8">
        ...
        <form action="{{ route('admin.drivers.status', $application->id) }}" method="POST">
            ...
            <div class="flex gap-4">
                ...
                <form action="{{ route('admin.drivers.destroy', $application->id) }}" method="POST" class="flex-1" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette candidature ?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="w-full px-6 py-3 bg-red-500 text-white font-bold rounded-xl hover:bg-red-600 transition">
                        🗑️ Supprimer
